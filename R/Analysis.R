...
setwd("C:\\Users\\Saraye Tell\\Documents\\server\\ecpc\\ecpc\\Code application note") #set working directory to desired folder
pathResults <- ""
pathFigures <- ""
...
zm<-rnorm(100, mean=0, sd=0.1)
type=2
h  <- 0.1516 
N<-100
L1<-function(t,zm,h){
  w<-c()
  for(i in 1:length(zm)) w[i]<-dnorm((t-zm[i])/h)/sum(dnorm((t-zm)/h))
  return(w)
}
kernel=function(type,value){
  if(type==1){
    if(abs(value)<=1){
      k<-0.5}
    else{k<-0}}
  if(type==2){
    k<-(1/sqrt(2*3.14))*exp(-1*(value^2)/2)} 
  if(type==3){
    if(abs(value)<=1){
      k<-0.75*(1-value^2)}
    else{k<-0}}
  if(type==4){
    if(abs(value)<=1){
      k<-(70/81)*((1-abs(value)^3)^3)}
    else{k<-0}}
  k}
CV=GCV=h=numeric(N)
L=matrix(0,ncol=n,nrow=n)
seq1<-seq(from=1,to=100,by=1)
for(m in 1:N){
  h[m]=seq1[m]
  rhat=numeric(n)
  for(i in 1:n){
  ...
  v=sum(diag(L))
  for(i in 1:n){
    GCV[m]=GCV[m]+(((Y[i]-rhat[i])/(1-L[i,i]))^2)/n
    CV[m]=CV[m]+(((Y[i]-rhat[i])/(1-v/n))^2)/n
  }
}
alpha0<-rhat
beta <- rnorm(p, mean=0, sd=0.1) #simulate effects
X <- matrix(rnorm(n*p,mean=0,sd=1), n,p) #simulate observed training data
Y <- rnorm(n,mean = X%*%beta + alpha0, sd = 1) #simulate response training data
X2 <- matrix(rnorm(n2*p,mean=0,sd=1), n,p) #simulate observed test data
Y2 <- rnorm(n2,mean = X2%*%beta + alpha0, sd = 1) #simulate response test data
...
for(m in 1:N){
  ...
  v=sum(diag(L))
  for(i in 1:n){
    GCV[m]=GCV[m]+(((Dat$Y[i]-rhat[i])/(1-L[i,i]))^2)/n
    CV[m]=CV[m]+(((Dat$Y[i]-rhat[i])/(1-v/n))^2)/n
  }
}
alpha0<-rhat
Dat$alpha0<-alpha0
Dat$beta <- rnorm(p, mean=0, sd=0.1) #simulate effects
Dat$Xctd <- matrix(rnorm(n*p,mean=0,sd=1), n,p) #simulate observed training data
Dat$Y <- rnorm(n,mean = Dat$Xctd%*%Dat$beta + Dat$alpha0, sd = 1) #simulate response training data
Dat$X2ctd <- matrix(rnorm(n2*p,mean=0,sd=1), n,p) #simulate observed test data
Dat$Y2 <- rnorm(n2,mean = Dat$X2ctd%*%Dat$beta + Dat$alpha0, sd = 1) #simulate response test data
...
#Dat <- list()
#Dat$alpha0<-alpha0
#Dat$beta <- rnorm(p,sd=0.1)
#Dat$Xctd <- matrix(rnorm(n*p) ,n,p)
#Dat$X2ctd <- matrix(rnorm(n*p) ,n,p)
#Dat$Y <- rnorm(n,mean=Dat$Xctd%*%Dat$beta+ Dat$alpha0,sd=1)
#Dat$Y2 <- rnorm(n,mean=Dat$X2ctd%*%Dat$beta+ Dat$alpha0,sd=1)
...
hierarchy.grouplevel <- obtainHierarchy(groupset = groupingHierarchical) #obtain grouping on group level that defines the hierarchy
visualiseGroupset(Groupset = groupingHierarchical,groupset.grouplvl = hierarchy.grouplevel) #visualise hierarchical groups
#Fit ecpc----
...
tic<-proc.time()[[3]]
fit <- ecpc(Y=Dat$Y,X=Dat$Xctd,groupsets=list(groupingRandom,groupingInformative,groupingHierarchical),
            groupsets.grouplvl=list(NULL,NULL,hierarchy.grouplevel),
...
#                groupsets=list(groupingRandom,groupingInformative,groupingHierarchical),
#                 groupsets.grouplvl=list(NULL,NULL,hierarchy.grouplevel),
#                 hypershrinkage=c("ridge","ridge","hierLasso,ridge"),
#                model="linear",maxsel=c(5,10,15,20))
#toc <- proc.time()[[3]]-tic
...
visualiseGroupsetweights(dfGrps = cv.fit$dfGrps,GroupsetNames = c("random","informative","hierarchical"),boxplot=F)

#Plot estimated group weights across folds
#Grouping 1:
visualiseGroupweights(dfGrps = cv.fit$dfGrps[cv.fit$dfGrps$Groupset==1,],Groupset=groupingRandom,boxplot = F)

#Grouping 2:
visualiseGroupweights(dfGrps = cv.fit$dfGrps[cv.fit$dfGrps$Groupset==2,],Groupset=groupingInformative,boxplot = F)

#Grouping 3; plot group weight versus continuous co-data value:
visualiseGroupweights(dfGrps = cv.fit$dfGrps[cv.fit$dfGrps$Groupset==3,],Groupset=groupingHierarchical,
                      values=continuousCodata,groupset.grouplvl = hierarchy.grouplevel,boxplot = F)
#visualise which hierarchical groups are selected in a specific fold:
fold <- 1
visualiseGroupset(Groupse = groupingHierarchical,groupset.grouplvl = hierarchy.grouplevel,
                  groupweights = cv.fit$dfGrps$Group.weight[cv.fit$dfGrps$Groupset==3&cv.fit$dfGrps$Fold==fold])

#########################################################
alpha0
Dat$beta_New<- fit$beta
Dat$beta <- Dat$beta_New #simulate effects
Dat$alpha_had<- Dat$Y - Dat$Xctd%*%Dat$beta
Dat$Xctd <- matrix(rnorm(n*p,mean=0,sd=1), n,p) #simulate observed training data
Dat$Y <- rnorm(n,mean = Dat$Xctd%*%Dat$beta + Dat$alpha_had, sd = 1) #simulate response training data
Dat$X2ctd <- matrix(rnorm(n2*p,mean=0,sd=1), n,p) #simulate observed test data
Dat$Y2 <- rnorm(n2,mean = Dat$X2ctd%*%Dat$beta + Dat$alpha_had, sd = 1) #simulate response test data
...
Dat$beta_New<- fit$beta
Dat$Xctd <- matrix(rnorm(n*p,mean=0,sd=1), n,p) #simulate observed training data
Dat$Y <- rnorm(n,mean = Dat$Xctd%*%Dat$beta_New + Dat$alpha0, sd = 1) #simulate response training data
Dat$X2ctd <- matrix(rnorm(n2*p,mean=0,sd=1), n,p) #simulate observed test data
Dat$Y2 <- rnorm(n2,mean = Dat$X2ctd%*%Dat$beta_New + Dat$alpha0, sd = 1) #simulate response test data
K<-matrix( nrow=n, ncol=n)
#rhat<-matrix(NA, nrow=n, ncol=1)
#alpha0<-rhat
b_s<-1
a_s<-2.01
b_t<-10
...
C1<-10^(-5)
sigma<-1
z<-rnorm(100, mean=0, sd=0.1)
h  <- 0.1516
# GaussianKernel
GK <- function(u) {(1/sqrt(2*pi))*exp(-(u^2)/2)} # or dnorm(u)
zp<-rnorm(10000, mean=0, sd=0.1)
t<-GK(zp)
K<-matrix( t,nrow=n, ncol=n)
#dth-order polynomial kernel
#library(kernlab)
#polydot(zp,degree = 1, scale = 1, offset = 1)
n <- 100
I<-diag(n)
alpha_u<-matrix(nrow=n, ncol=10)
delta<-runif(n,0.0001, 0.01)
#gibbs <- function(n.sims, a_t,b_t, alpha.start,tau.start,Dat$Y, Dat$Xctd, a_s, b_s) {
# n.sims <- n
#alpha.draws <- matrix(NA, nrow=n.sims, ncol=1)
#  sigma.draws<- c()
#  tau.draws<- c()
# alpha.cur <- alpha.start
# tau.cur <- tau.start
# for (i in 1:n.sims) {
#sigma.update <- function(a_s, b_s, alpha) {
#rinvgauss(1, mean = n/2 + a_s, disp = (t(Dat$Y - Dat$Xctd%*%Dat$beta_New - alpha)%*%(Dat$Y - Dat$Xctd%*%Dat$beta_New - alpha)/2) + b_s)
#}
#alpha.update <- function(sigma,tau) {
#  rnorm(n, mean = (sigma**(-2)) %*% ((tau%*%K)**(-1) + sigma**(-2))**(-1) %*% (Dat$Y - Dat$Xctd%*%Dat$beta_New), sd = ((tau %*%K)**(-1) + sigma**(-2))**(-1))
#}
#tau.update <- function(a_t,b_t, alpha) {
# rinvgauss(1, mean = n/2 + a_t, disp = (t(alpha)%*%(K)^(-1)%*%t(alpha)/2 + b_t)^(-1)) 
#}
#for (i in 1:n.sims) {
# sigma.cur <- sigma.update(a_s, b_s, alpha.cur)
#  alpha.cur <- alpha.update(sigma.cur,tau.cur)
# tau.cur <- tau.update(a_t,b_t, alpha.cur)
#if (i > burnin & (i - burnin) %% thin == 0) {
# sigma.draws <- sigma.cur
# alpha.draws <- alpha.cur
# tau.draws <- tau.cur
# }
#}
#return (list(sigma.draws,alpha.draws ,tau.draws) )
#}
#gibbs(n.sims, a_t,b_t, alpha.start,tau.start,Dat$Y, Dat$Xctd, a_s, b_s)
#res <- matrix(nrow = niter, ncol = 2)
#result2 <- matrix(nrow = n, ncol = niter)
Y<-Dat$Y
X<-Dat$Xctd
function(a_t,b_t,Y, X, a_s, b_s,niter = 1000) {
  
  sigma<- 1
  alpha <- alpha0
  tau<-0.21
  for (i in 1:100) {
    T1<-t(Y - X%*%Dat$beta_New - alpha)
    T2<-Y - X%*%Dat$beta_New - alpha
    disp1 = (T1%*%T2)/2 + b_s
    sigma[i] <-  rinvgauss(1, mean = n/2 + a_s, disp = disp1)
  }
  sigma
}
...
Nsim=1000
alpha1<-matrix(nrow=n, ncol=Nsim)
tau=rep(0,Nsim)                  #init arrays
alpha1 <-  alpha0
K1<-t(alpha1)%*%(K^(-1))%*%alpha1
tau[1]  <-  rinvgauss(1, mean = n/2 + a_t, disp = (K1/2 + b_t)^(-1))
for (i in 2:Nsim){
  K1<-t(alpha1)%*%(K^(-1))%*%alpha1
  tau[i]  <-  rinvgauss(1, mean = n/2 + a_t, disp = (K1/2 + b_t)^(-1))
  Q1<-1/(sigma^(2)+delta)
  Q2<-((tau[i]*K)**(-1) + (1/(sigma^(2)+delta))*I)**(-1)
  alpha1 <-  rnorm(n, mean = Q1 * Q2 %*% (Y - X%*%Dat$beta_New), sd = Q2)
  
  alphau<-matrix( alpha1,nrow=n, ncol=Nsim)
}

mean(tau)


Dat$alpha0<-rowMeans(alphau)
Dat$beta<-fit$beta
#Dat$beta <- rnorm(p, mean=0, sd=0.1) #simulate effects
Dat$Xctd <- matrix(rnorm(n*p,mean=0,sd=1), n,p) #simulate observed training data
Dat$Y <- rnorm(n,mean = Dat$Xctd%*%Dat$beta + Dat$alpha0, sd = 1) #simulate response training data
...
tic<-proc.time()[[3]]
fit <- ecpc(Y=Dat$Y,X=Dat$Xctd,groupsets=list(groupingRandom,groupingInformative,groupingHierarchical),
            groupsets.grouplvl=list(NULL,NULL,hierarchy.grouplevel),intrcpt=FALSE,
            hypershrinkage=c("ridge","ridge","hierLasso,ridge"),
            model="linear",maxsel=c(5,10,15,20),
            Y2=Dat$Y2,X2=Dat$X2ctd)
toc <- proc.time()[[3]]-tic
...
cv.fit<- cv.ecpc(type.measure="MSE",outerfolds=10,
                 ...
                 groupsets=list(groupingRandom,groupingInformative,groupingHierarchical),
                 groupsets.grouplvl=list(NULL,NULL,hierarchy.grouplevel),
                 hypershrinkage=c("ridge","ridge","hierLasso,ridge"),balance=F,
                 model="linear",maxsel=c(5,10,15,20))
toc <- proc.time()[[3]]-tic
...
alpha_had<-Dat$alpha0
beta_had<- Dat$beta
X <- matrix(rnorm(n*p,mean=0,sd=1), n,p) #simulate observed training data
Y <- rnorm(n,mean = X%*%beta_had + alpha_had, sd = 1) #simulate response training data
X2 <- matrix(rnorm(n2*p,mean=0,sd=1), n,p) #simulate observed test data
Y2 <- rnorm(n2,mean = X2%*%beta_had + alpha_had, sd = 1) #simulate response test data
#Consider two co-data variables----
Z1 <- abs(beta) #informative co-data
Z2 <- rnorm(p,mean=0,sd=1) #random, non-informative co-data
Z <- cbind( Z1 , Z2 ) #(px2)-dimensional co-data matrix
...
#Fit model----
fit <- ecpc(Y,X,Z=list(Z),X2=X2,Y2=Y2, postselection=FALSE,intrcpt=FALSE)
summary(fit$beta) #fitted regression coefficients
fit$gamma*fit$tauglobal #fitted co-data weights
...
fit.gam <- ecpc(Y, X, Z = Z.all, paraPen = paraPen.all, intrcpt.bam=TRUE, intrcpt=FALSE,
...
fit.scam <- ecpc(Y, X, Z = Z.all, paraPen = paraPen.all, paraCon = paraCon,intrcpt=FALSE, 
                 X2=X2, Y2=Y2, postselection=FALSE)
fit.scam$MSEecpc
fit.scam$MSEridge
...
Dat$Xctd <- matrix(rnorm(n*p) ,n,p)
Dat$X2ctd <- matrix(rnorm(n*p) ,n,p)
Dat$Y <- rnorm(n,mean=c(Dat$Xctd%*%Dat$beta + alpha_had),sd=sigma)
...
library(ggplot2)
library(kernlab)
R<-polydot(degree = 1, scale = 1, offset = 1)
K<-kernelMatrix(R, zp)
dim(K)
